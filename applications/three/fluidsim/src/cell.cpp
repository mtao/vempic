...
size_t FluidVEM3Cell::sample_size() const { return indexer().sample_size(); }
// the number of DOFs in the local sample
size_t FluidVEM3Cell::local_sample_size() const {
    return point_sample_size() + moment_size();
}

// size_t FluidVEM3Cell:: edge_count() const{}
size_t FluidVEM3Cell::edge_interior_sample_count() const {
    size_t size = 0;
...
size_t FluidVEM3Cell::point_sample_size() const {
    return vertex_count() + edge_interior_sample_count();
}
size_t FluidVEM3Cell::moment_degree() const {
    return moment_indexer().degree(cell_index());
}
size_t FluidVEM3Cell::moment_size() const {
    return moment_indexer().num_coefficients(cell_index());
}
size_t FluidVEM3Cell::monomial_degree() const {
    return monomial_indexer().degree(cell_index());
}
size_t FluidVEM3Cell::monomial_size() const {
    return monomial_indexer().num_coefficients(cell_index());
}

size_t FluidVEM3Cell::local_moment_index_offset() const {
    return point_sample_size();
}
size_t FluidVEM3Cell::global_moment_index_offset() const {
    return point_sample_indexer().num_coefficients() +
           moment_indexer().coefficient_offset(cell_index());
}
size_t FluidVEM3Cell::global_monomial_index_offset() const {
    return _indexer.monomial_indexer().coefficient_offset(cell_index());
}

mtao::MatXd FluidVEM3Cell::monomial_l2_grammian() const {
    return monomial_l2_grammian(monomial_degree());
}
mtao::MatXd FluidVEM3Cell::monomial_dirichlet_grammian() const {
    return monomial_dirichlet_grammian(monomial_degree());
}

mtao::MatXd FluidVEM3Cell::l2_projector() const {
...
mtao::MatXd FluidVEM3Cell::dirichlet_projector() const {
...
mtao::MatXd FluidVEM3Cell::l2_sample_projector() const {
    auto Pis = l2_projector();
    auto B = monomial_evaluation();
    return B * Pis;
}
mtao::MatXd FluidVEM3Cell::dirichlet_sample_projector() const {
    auto Pis = dirichlet_projector();
    auto B = monomial_evaluation();
    return B * Pis;
}

mtao::MatXd FluidVEM3Cell::sample_monomial_l2_grammian() const {
    ...
    double vol = volume();
    // auto integrals =
    //    monomial_indexer().monomial_integrals(index, 3 * moment_degree());
    size_t mom_size = moment_size();
    size_t mom_off = local_moment_index_offset();
    // for row 1... n_{k-3} we use the moment inner product
    // n_{k-3} = moment size
    ...
    size_t off = moment_size();  // k-3 polynomials
...
mtao::MatXd FluidVEM3Cell::sample_monomial_dirichlet_grammian() const {
    mtao::MatXd R(local_sample_size(), monomial_size());
    R.setZero();

    std::map<size_t, size_t> w3l_pi = world_to_local_point_sample_indices();
    ...
    for (auto&& [eidx, sgn] : edges()) {
        ...
        for (size_t poly_idx = 1; poly_idx < monomial_size(); ++poly_idx) {
            ...
            for (auto&& [vidx, weight] : mtao::iterator::zip(edge_indices, W)) {
                size_t local_index = w3l_pi.at(vidx);
                auto p =
                ...
                double dmdn = (xc * p.array().pow(xexp.array()).prod() +
                              ...
                              diameter;
                // factor of 3 due to gauss lobatto being defined on [-1,1]
                R(local_index, poly_idx) += dmdn * weight * edge_length / 3;
            }
        }
    }
    double vol = volume();
    ...
    double d3 = std::pow(diameter, 3.0);
    // std::cout <<"Laplacian\n" << L << std::endl;
    for (int o = 0; o < L.outerSize(); ++o) {
        for (Eigen::SparseMatrix<double>::InnerIterator it(L, o); it; ++it) {
            int row = it.row() + mom_off;
            int col = it.col();
            R(row, col) = -it.value() * vol / d3;
        }
    }
    return R;
}

mtao::MatXd FluidVEM3Cell::regularized_monomial_dirichlet_grammian() const {
...
mtao::MatXd FluidVEM3Cell::l2_projector_error() const {
    auto Pi = l2_sample_projector();
    Pi.noalias() = mtao::MatXd::Identity(Pi.rows(), Pi.cols()) - Pi;
    return Pi;
}
mtao::MatXd FluidVEM3Cell::dirichlet_projector_error() const {
    auto Pi = dirichlet_sample_projector();
    Pi.noalias() = mtao::MatXd::Identity(Pi.rows(), Pi.cols()) - Pi;
    return Pi;
}

std::vector<size_t> FluidVEM3Cell::point_sample_indices() const {
    auto s = point_sample_indexer().cell_indices(cell_index());
    return {s.begin(), s.end()};
}

std::vector<size_t> FluidVEM3Cell::sample_indices() const {
...
mtao::MatXd FluidVEM3Cell::monomial_evaluation() const {
    mtao::MatXd R(local_sample_size(), monomial_size());
    R.setZero();
    double vol = volume();
...
std::map<size_t, size_t> FluidVEM3Cell::world_to_local_point_sample_indices()
    const {
...
std::map<size_t, size_t> FluidVEM3Cell::world_to_local_sample_indices() const {
...
FluidVEM3Cell::local_to_world_monomial_indices() const {
    return monomial_indexer().coefficient_indices(cell_index());
}
std::vector<size_t> FluidVEM3Cell::local_to_world_point_sample_indices() const {
    return point_sample_indices();
}
std::vector<size_t> FluidVEM3Cell::local_to_world_sample_indices() const {
    return sample_indices();
}
Eigen::SparseMatrix<double> FluidVEM3Cell::local_to_world_monomial_map() const {
...
Eigen::SparseMatrix<double> FluidVEM3Cell::local_to_world_sample_map() const {
