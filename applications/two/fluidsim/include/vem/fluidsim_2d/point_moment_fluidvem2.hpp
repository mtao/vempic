...
struct PointMomentFluidVEM2;
template <>
struct FluidVEM2Traits<PointMomentFluidVEM2> {
    using CellType = PointMomentVEM2Cell;
    using IndexerType = PointMomentIndexer;
};

struct PointMomentFluidVEM2 : public FluidVEM2Base<PointMomentFluidVEM2> {
    using Base = FluidVEM2Base<PointMomentFluidVEM2>;
    ...
    // PointMomentFluidVEM2(const VEMMesh2 &_mesh, size_t velocity_max_degree);
    using Base::Base;
    ...
    using Base::polynomial_to_sample_evaluation_matrix;
    ...
    using Base::sample_to_poly_codivergence;

    // pressure sample -> velocity sample
    using Base::sample_gradient;
