...
MonomialBasisIndexer<D, E>::MonomialBasisIndexer(const MeshType &mesh,
    ...
    : PartitionedCoefficientIndexer(
          mesh.cell_count(), polynomials::num_monomials_upto<D>(max_degree)),
      _mesh(mesh) {
    if constexpr (D == E) {
...
MonomialBasisIndexer<D, E>::MonomialBasisIndexer(
    const MeshType &mesh, std::vector<size_t> per_degrees,
...
void MonomialBasisIndexer<D, E>::fill_diameters() {
    ...
    // spdlog::info("Final diams: {}", fmt::join(_diameters, ","));
}
...
template <int D, int E>
mtao::VecXd MonomialBasisIndexer<D, E>::monomial_integrals(
    size_t cell_index, int max_degree) const {
    return scaled_monomial_cell_integrals(_mesh, cell_index,
                                          diameter(cell_index), max_degree);
}
