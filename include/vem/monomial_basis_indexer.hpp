...
namespace detail {
    ...
    class MonomialBasisIndexer : public PartitionedCoefficientIndexer {
      public:
        ...
        MonomialBasisIndexer(const MeshType &mesh, std::vector<size_t> degrees, std::vector<double> diameters = {});
        ...
        double evaluate_monomial(size_t cell, size_t index, const Eigen::MatrixBase<Derived> &P) const;

        template<typename Derived>
        Vec evaluate_monomial_gradient(size_t cell, size_t index, const Eigen::MatrixBase<Derived> &P) const;
