...
size_t PointMomentVEM2Cell::sample_size() const {
    return _indexer->sample_size();
}
// the number of DOFs in the local sample
size_t PointMomentVEM2Cell::local_sample_size() const {
    return point_sample_size() + moment_size();
}

const MonomialBasisIndexer& PointMomentVEM2Cell::monomial_indexer() const {
...
// size_t PointMomentVEM2Cell:: edge_count() const{}
size_t PointMomentVEM2Cell::edge_interior_sample_count() const {
    size_t size = 0;
...
size_t PointMomentVEM2Cell::point_sample_size() const {
    return vertex_count() + edge_interior_sample_count();
}
size_t PointMomentVEM2Cell::moment_degree() const {
    return moment_indexer().degree(cell_index());
}
size_t PointMomentVEM2Cell::moment_size() const {
    return moment_indexer().num_coefficients(cell_index());
}
size_t PointMomentVEM2Cell::monomial_degree() const {
    return monomial_indexer().degree(cell_index());
}
size_t PointMomentVEM2Cell::monomial_size() const {
    return monomial_indexer().num_coefficients(cell_index());
}

size_t PointMomentVEM2Cell::local_moment_index_offset() const {
    return point_sample_size();
}
size_t PointMomentVEM2Cell::moment_only_global_moment_index_offset() const {
    return moment_indexer().coefficient_offset(cell_index());
}
size_t PointMomentVEM2Cell::global_moment_index_offset() const {
    return point_sample_indexer().num_coefficients() +
           moment_only_global_moment_index_offset();
}
size_t PointMomentVEM2Cell::global_monomial_index_offset() const {
    return _indexer->monomial_indexer().coefficient_offset(cell_index());
}

mtao::MatXd PointMomentVEM2Cell::monomial_l2_grammian() const {
    return monomial_l2_grammian(monomial_degree());
}
mtao::MatXd PointMomentVEM2Cell::monomial_dirichlet_grammian() const {
    return monomial_dirichlet_grammian(monomial_degree());
}

mtao::MatXd PointMomentVEM2Cell::l2_projector() const {
...
mtao::MatXd PointMomentVEM2Cell::dirichlet_projector() const {
...
mtao::MatXd PointMomentVEM2Cell::l2_sample_projector() const {
    auto Pis = l2_projector();
    auto B = monomial_evaluation();
    return B * Pis;
}
mtao::MatXd PointMomentVEM2Cell::dirichlet_sample_projector() const {
    auto Pis = dirichlet_projector();
    auto B = monomial_evaluation();
    return B * Pis;
}

mtao::MatXd PointMomentVEM2Cell::sample_monomial_l2_grammian() const {
...
mtao::MatXd PointMomentVEM2Cell::sample_monomial_dirichlet_grammian() const {
    mtao::MatXd R(local_sample_size(), monomial_size());
    R.setZero();
...
mtao::MatXd PointMomentVEM2Cell::regularized_monomial_dirichlet_grammian()
    const {
...
mtao::MatXd PointMomentVEM2Cell::l2_projector_error() const {
    auto Pi = l2_sample_projector();
    Pi.noalias() = mtao::MatXd::Identity(Pi.rows(), Pi.cols()) - Pi;
    return Pi;
}
mtao::MatXd PointMomentVEM2Cell::dirichlet_projector_error() const {
    auto Pi = dirichlet_sample_projector();
    Pi.noalias() = mtao::MatXd::Identity(Pi.rows(), Pi.cols()) - Pi;
    return Pi;
}

std::vector<size_t> PointMomentVEM2Cell::point_sample_indices() const {
    auto s = point_sample_indexer().cell_indices(cell_index());
    return {s.begin(), s.end()};
}

std::vector<size_t> PointMomentVEM2Cell::sample_indices() const {
...
mtao::MatXd PointMomentVEM2Cell::monomial_evaluation() const {
    mtao::MatXd R(local_sample_size(), monomial_size());
    R.setZero();
    double vol = volume();
...
PointMomentVEM2Cell::world_to_local_point_sample_indices() const {
...
std::map<size_t, size_t> PointMomentVEM2Cell::world_to_local_sample_indices()
    const {
...
PointMomentVEM2Cell::local_to_world_monomial_indices() const {
    return monomial_indexer().coefficient_indices(cell_index());
}

mtao::iterator::detail::range_container<size_t>
PointMomentVEM2Cell::moment_indices() const {
    return moment_indexer().coefficient_indices(cell_index());
}
std::vector<size_t> PointMomentVEM2Cell::local_to_world_point_sample_indices()
    const {
    return point_sample_indices();
}
std::vector<size_t> PointMomentVEM2Cell::local_to_world_sample_indices() const {
    return sample_indices();
}
Eigen::SparseMatrix<double> PointMomentVEM2Cell::local_to_world_monomial_map()
    const {
...
Eigen::SparseMatrix<double> PointMomentVEM2Cell::local_to_world_sample_map()
    const {
