...
#include "curve_drawing_viewer2d.hpp"
...
class VemViewer2d : public CurveDrawingViewer2d {
   public:
    int color_mode = 0;
    std::optional<mandoline::CutCellMesh<2>> ccm;
    // std::variant<nullptr_t, PointwiseVEMMesh2, DIWLevinVEMMesh2> vem;
    std::unique_ptr<PointwiseVEMMesh2> vem;
    std::shared_ptr<Sim> sim;
    SimVis sim_vis;
    // enum class VEMType : int {
    //    DIWLevin = 0,
    //    DIWLevinIntegrated = 1,
    //    Pointwise = 2
    //};
    ...
    // VEMType vem_type = VEMType::Pointwise;
    MeshBCType mesh_bc_type = MeshBCType::LinearDirichlet;
    // std::optional<DIWLevinVEMMesh2> vem;
...
void VemViewer2d::gui() {
    ...
    if (ccm) {
        ...
        //}
        {
            const char *items[] = {"None", "Dirichlet", "Neumann",
                                   "LinearDirichlet"};
            if (ImGui::Combo("Boundary condition type",
                             reinterpret_cast<int *>(&mesh_bc_type), items,
                             // if (ImGui::Combo("VEM Type", static_cast<int
                             // *>(&vem_type), items,
                             4)) {
                // vem.emplace<nullptr_t>();
    ...
    if (histogram.size() > 0) {
        ...
        ImGui::PlotHistogram("value histogram", histogram.data(),
                             histogram.size(), 0, nullptr, FLT_MIN, FLT_MAX,
                             ImVec2(0, 80.f));
...
void VemViewer2d::solve_vem() {
    ...
    if (mesh_bc_type == MeshBCType::Dirichlet ||
        mesh_bc_type == MeshBCType::LinearDirichlet) {
            ...
            if (f.is_mesh_face()) {
                for (auto &&loop : f.indices) {
                    std::copy(loop.begin(), loop.end(),
                              std::inserter(vertices, vertices.end()));
                }
            }
        }
        for (auto &&v : vertices) {
...
void VemViewer2d::update_faces() {
    ...
    if (vertex_valued_cell_colors) {
        ...
        spdlog::error("nV/nF/nC: {} {} {}", verts.cols(), F.cols(),
                      colors.cols());
